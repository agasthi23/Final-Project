...
import { FiMail, FiLock, FiArrowRight, FiZap, FiDroplet, FiTrendingUp, FiShield, FiEye, FiEyeOff } from 'react-icons/fi';
...
const Login = () => {
 ...
 const handleSubmit = async (e) => {
  ...
  try {
    ...
    console.log('1. Full result:', result);
    console.log('2. result.success:', result.success);
    console.log('3. result.user:', result.user);
    console.log('4. result.user.role:', result?.user?.role);

    if (result.success) {
      console.log('5. Inside success block');
      ...
      <div style={{ flex:"0 0 42%", background:"linear-gradient(145deg,#0f172a 0%,#1e293b 60%,#0f2d5a 100%)",
        ...
        <div style={{ position:"relative", zIndex:1 }}>
          <div className="au-si1" style={{ display:"inline-block", padding:"5px 14px",
            borderRadius:999, background:"rgba(37,99,235,.2)", border:"1px solid rgba(37,99,235,.35)",
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            {features.map((f,i) => (
              <div key={i} className={`au-feat au-si${Math.min(i+1,3)}`}
                style={{ display:"flex", alignItems:"center", gap:14, transition:"transform .2s ease" }}>
                <div style={{ width:38, height:38, borderRadius:10,
                  background:`${f.color}18`, border:`1px solid ${f.color}33`,
                  display:"flex", alignItems:"center", justifyContent:"center",
                  color:f.color, flexShrink:0 }}>{f.icon}</div>
                <span style={{ fontSize:"0.85rem", color:"rgba(255,255,255,.7)", fontWeight:500 }}>{f.text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ width:"100%", maxWidth:420 }}>
          ...
          <div className="au-fu1" style={{ marginBottom:32 }}>
            <h1 style={{ fontSize:"1.75rem", fontWeight:800, color:"#0f172a",
              margin:"0 0 6px", letterSpacing:"-0.03em" }}>Welcome back</h1>
          ...
          {location.state?.message && (
            <div className="au-fu2" style={{ padding:"12px 16px", borderRadius:12, marginBottom:20,
          ...
          <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:18 }}>
            ...
            <div className="au-fu2" style={{ display:"flex", flexDirection:"column", gap:6 }}>
              <label style={{ fontSize:"0.75rem", fontWeight:700, color:"#475569",
                textTransform:"uppercase", letterSpacing:"0.07em" }}>Email Address</label>
            ...
            <div className="au-fu3" style={{ display:"flex", flexDirection:"column", gap:6 }}>
              <label style={{ fontSize:"0.75rem", fontWeight:700, color:"#475569",
                textTransform:"uppercase", letterSpacing:"0.07em" }}>Password</label>
              <div style={{ position:"relative" }}>
                ...
                <input
                  ...
                  style={{ width:"100%", padding:"12px 44px 12px 42px",
                    ...
                    color:"#0f172a", transition:"all .15s", boxSizing:"border-box" }}/>
                {/* Eye toggle */}
                <button type="button" className="au-eye"
          ...
          <p className="au-fu5" style={{ textAlign:"center", fontSize:"0.875rem", color:"#64748b", margin:0 }}>
